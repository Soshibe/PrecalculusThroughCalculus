...
// NOTE: Renamed EPSILON to EPSILON_SF_PARSER to avoid potential conflicts if other modules define their own EPSILON
static constexpr double EPSILON_SF_PARSER = 1e-9;
...
// This is the public interface for parsing
export NodePtr parse_function_string(const std::string& expression) {
...
NodePtr simplify_recursive(NodePtr node); // Not exported, internal helper for simplify
...
bool are_equal(NodePtr n1, NodePtr n2) {
    ...
    case FuncType::Constant:
        return std::fabs(n1->value - n2->value) < EPSILON_SF_PARSER;
...
struct NodePtrCmp {
    bool operator()(NodePtr const& a, NodePtr const& b) const {
        ...
        if (a->type == FuncType::Constant) {
            ...
            return a->value < b->value - EPSILON_SF_PARSER;
        }
...
bool is_constant_val(NodePtr node, double val) {
    return node && node->type == FuncType::Constant && std::fabs(node->value - val) < EPSILON_SF_PARSER;
}
...
// For `-x`, it's `(-1.0, x_NodePtr)`
// For `-(2*x)`, it's `(-2.0, x_NodePtr)`
export std::vector<std::pair<double, NodePtr>> flatten_and_extract_terms(NodePtr node) {
...
NodePtr collect_like_terms(NodePtr node, bool& changed) {
    ...
    for (const auto& term_pair : flat_terms) {
        ...
        if (base && base->type == FuncType::Constant && std::fabs(base->value - 1.0) < EPSILON_SF_PARSER) {
    ...
    for (const auto& pair : grouped_terms) {
        ...
        if (std::fabs(coeff) < EPSILON_SF_PARSER) { // If coefficient became zero after combining (e.g., x - x)
            local_changed = true;
            continue;
        }

        NodePtr term_to_add;
        if (base->type == FuncType::Constant && std::fabs(base->value - 1.0) < EPSILON_SF_PARSER) {
            // This is the accumulated constant sum
            term_to_add = make_const(coeff); // The coefficient IS the constant value
        }
        else if (std::fabs(coeff - 1.0) < EPSILON_SF_PARSER) {
            term_to_add = base;
        }
        else if (std::fabs(coeff - (-1.0)) < EPSILON_SF_PARSER) {
...
NodePtr simplify_once(NodePtr node, bool& changed) {
    ...
    for (const auto& c : node->children) {
        ...
        // NEW MODIFICATION: If a child is a constant and its value is less than EPSILON,
        // convert it to a constant 0.0. This *modifies* the value.
        if (simplified_c && simplified_c->type == FuncType::Constant &&
            std::fabs(simplified_c->value) < EPSILON_SF_PARSER) {
            // Only create a new node if the value is meaningfully different from 0.0
            // This avoids unnecessary node creation if it was already 0.0
            if (std::fabs(simplified_c->value) > 0.0) { // Check if it's not already exactly 0.0
                simplified_c = make_const(0.0);
                children_changed = true; // Mark as changed because a child's value was normalized
            }
        }
    ...
    // NEW MODIFICATION: Apply the same epsilon-to-zero clamping to the current node if it's a constant.
    if (node->type == FuncType::Constant) {
        if (std::fabs(node->value) < EPSILON_SF_PARSER && std::fabs(node->value) > 0.0) { // Check if it's not already exactly 0.0
            changed = true; // Mark as changed because the node's value was normalized
            return make_const(0.0); // Return a new node with the normalized value
        }
        return node; // No change needed or already exactly 0.0
    }
        ...
        node->children[1]->type == FuncType::Constant)
    {
        ...
        case FuncType::Div:
            if (std::fabs(rhs) < EPSILON_SF_PARSER) throw std::runtime_error("Division by zero in simplification");
            return make_const(lhs / rhs);
        case FuncType::Pow:
            if (lhs < 0.0 && std::fabs(std::floor(rhs) - rhs) > EPSILON_SF_PARSER) // fractional exponent of negative base
    ...
    if (node->children.size() == 1 && node->children[0]->type == FuncType::Constant) {
        ...
        case FuncType::Tan:
            ...
            if (std::fabs(std::fmod(std::fabs(val), M_PI) - M_PI_2) < EPSILON_SF_PARSER)
                throw std::runtime_error("tan(x) undefined at pi/2 + n*pi in constant folding");
            changed = true; return make_const(std::tan(val));
        case FuncType::Ln:
            if (val <= EPSILON_SF_PARSER) throw std::runtime_error("ln(x) undefined for x <= 0 in constant folding");
            changed = true; return make_const(std::log(val));
        case FuncType::Log:
            if (val <= EPSILON_SF_PARSER) throw std::runtime_error("log(x) undefined for x <= 0 in constant folding");
    ...
    case FuncType::Div: {
        ...
        if (is_constant_val(denominator, 1.0)) { changed = true; return numerator; }
        if (is_constant_val(numerator, 0.0) && !is_constant_val(denominator, 0.0)) {
            changed = true; return make_const(0.0);
        }
        if (is_constant_val(denominator, -1.0)) { changed = true; return make_unary(FuncType::Neg, numerator); }
        if (are_equal(numerator, denominator) && !is_constant_val(numerator, 0.0)) {
            changed = true; return make_const(1.0);
        }

        // NEW RULE: (A * B) / B = A  or (B * A) / B = A
        // Requires numerator to be a multiplication.
        if (numerator->type == FuncType::Mul && denominator->type != FuncType::Constant) { // Don't apply if denominator is constant, handled by constant folding
            NodePtr num_left = numerator->children[0];
            NodePtr num_right = numerator->children[1];

            if (are_equal(num_left, denominator) && !is_constant_val(num_left, 0.0)) { // (B * A) / B = A
                changed = true;
                return num_right; // Return A
            }
            if (are_equal(num_right, denominator) && !is_constant_val(num_right, 0.0)) { // (A * B) / B = A
                changed = true;
                return num_left; // Return A
            }
        }

        // NEW RULE: A / (A * B) = 1 / B
        // Requires denominator to be a multiplication.
        if (denominator->type == FuncType::Mul && numerator->type != FuncType::Constant) {
            NodePtr den_left = denominator->children[0];
            NodePtr den_right = denominator->children[1];

            if (are_equal(numerator, den_left) && !is_constant_val(numerator, 0.0)) { // A / (A * B) = 1 / B
                changed = true;
                return make_op("/", make_const(1.0), den_right);
            }
            if (are_equal(numerator, den_right) && !is_constant_val(numerator, 0.0)) { // A / (B * A) = 1 / B
                changed = true;
                return make_op("/", make_const(1.0), den_left);
            }
        }
        // More complex rules might involve factoring common terms.
    ...
    case FuncType::Pow: {
        ...
        if (is_constant_val(base, 0.0) && // 0^x = 0 (if x > 0)
            ...
            exponent->value > EPSILON_SF_PARSER) {
            changed = true; return make_const(0.0);
        }
...
// Exported `simplify` function
export NodePtr simplify(NodePtr node) {
    return simplify_recursive(node);
}

// Internal recursive simplification logic
export NodePtr simplify_recursive(NodePtr node) {
    if (!node) return nullptr;
    ...
    // NOTE: The constant normalization is now handled in simplify_once, so this base case is fine.
    ...
    do {
        ...
        if (!changed && !are_equal(previous_state_for_comparison, current)) {
            changed = true; // Force another iteration if a structural change occurred
        }
    } while (changed); // Continue as long as *any* simplification (direct or via children) occurred
